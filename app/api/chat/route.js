...
export async function POST(req) {
  try {
    ...
    // Only include system prompt if this is the first message (only one user message)
    const userMessages = messages.filter(m => m.role === 'user');
    const shouldIncludeSystem = userMessages.length === 1;
    
    const messagesWithSystem = shouldIncludeSystem 
